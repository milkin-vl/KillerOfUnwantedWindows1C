...
Написана в рамках поддержки пользователей моей [программы для администрирования баз 1С](https://helpme1c.ru/obnovlyator-1s-gruppovoe-paketnoe-obnovlenie-vsex-baz-za-odin-raz).
...
* Пользователь запустил базу и ушел, не дождавшись пока она откроется. Но база не открылась, так как появилось окно о том, что конфигурация отличается от конфигурации базы данных. Пока будет висеть эта загрузка, базу монопольно не заблокировать.
